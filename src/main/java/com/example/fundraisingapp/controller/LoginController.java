...
public class LoginController {
    ...
    public ResponseEntity<?> checkUser(
        ...
        if (userType.equals("fund seeker")) {
            boolean userExists = fundSeekerService.authenticate(email, password);
            if (userExists) {
                return new ResponseEntity<>(null, HttpStatus.OK);
            }
            return new ResponseEntity<>(null, HttpStatus.BAD_REQUEST);
        } else if (userType.equals("fund contributor")) {
            boolean userExists = fundContributorService.authenticate(email, password);
            if (userExists) {
                return new ResponseEntity<>(null, HttpStatus.OK);
            }
            return new ResponseEntity<>(null, HttpStatus.BAD_REQUEST);
        } else if (userType.equals("platform owner")) {
            boolean userExists = platformOwnerService.authenticate(email, password);
            if (userExists) {
                return new ResponseEntity<>(null, HttpStatus.OK);
            }
            return new ResponseEntity<>(null, HttpStatus.BAD_REQUEST);
